...
class Settings(BaseSettings):
    ...
    def readiness(self) -> tuple[list[str], list[str]]:
        ...
        if not all([self.cloudinary_cloud_name,self.cloudinary_api_key,self.cloudinary_api_secret]):warnings.append("Cloudinary is not configured; local media is ephemeral")
        if not self.smtp_host or not self.smtp_from_email:warnings.append("SMTP is not configured; account emails are disabled")
